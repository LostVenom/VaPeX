...
async def user_info(c: Titan, user, already=False):
    if not already:
...
async def chat_info(c: Titan, chat, already=False):
    u_name = False
    if not already:
        try:
...
@Titan.on_message(command(["info", "whois"]))
async def info_func(c: Titan, message: Message):
...
@Titan.on_message(command(["chinfo", "chatinfo", "chat_info"]))
async def chat_info_func(c: Titan, message: Message):
