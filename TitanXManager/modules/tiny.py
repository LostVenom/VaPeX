...
@register(pattern="^/mini ?(.*)")
async def _(event):
    reply = await event.get_reply_message()
    if not (reply and (reply.media)):
        await event.reply("`Please reply to a sticker`")
        return
    kontol = await event.reply("`Processing mini...`")
    ik = await tbot.download_media(reply)
    im1 = Image.open("TitanXManager/resources/blank_background.png")
    if ik.endswith(".tgs"):
...
__mod_name__ = "◈ 𝐌ɪɴɪ ◈"
__help__ = """
✧ /mini*:* Reply to any sticker or image to get its Mini version.
"""
