...
from TitanXRobot import (HELP_COMMANDS, LOGGER, PYROGRAM_VERSION, PYTHON_VERSION,
                    UPTIME, VERSION)
from TitanXRobot.bot_class import Titan
from TitanXRobot.utils.custom_filters import command
from TitanXRobot.utils.extras import StartPic
from TitanXRobot.utils.kbhelpers import ikb
from TitanXRobot.utils.start_utils import (gen_cmds_kb, gen_start_kb, get_help_msg,
...
@Titan.on_callback_query(filters.regex("^close_admin$"))
...
async def start(c: Titan, m: Message):
    ...
    if m.chat.type == ChatType.PRIVATE:
        ...
        try:
            cpt = f"""
Hey [{m.from_user.first_name}](http://t.me/{m.from_user.username})! I am Titan ✨.
...
@Titan.on_callback_query(filters.regex("^start_back$"))
async def start_back(_, q: CallbackQuery):
    try:
        cpt = f"""
Hey [{q.from_user.first_name}](http://t.me/{q.from_user.username})! I am Titan ✨.
I'm here to help you manage your group(s)!
Hit /help to find out more about how to use me in my full potential!
...
@Titan.on_callback_query(filters.regex("^commands$"))
...
@Titan.on_callback_query(filters.regex("^bot_curr_info$"))
async def give_curr_info(c: Titan, q: CallbackQuery):
    start = time()
...
@Titan.on_callback_query(filters.regex("^plugins."))
async def get_module_info(c: Titan, q: CallbackQuery):
