...
from TitanXRobot import (LOGGER, RMBG, Audd, genius_lyrics, is_audd,
                    is_genius_lyrics, is_rmbg)
from TitanXRobot.bot_class import Titan
from TitanXRobot.utils.custom_filters import command
from TitanXRobot.utils.http_helper import *
from TitanXRobot.utils.sticker_help import toimage
from TitanXRobot.utils.web_helpers import *

# @Titan.on_message(command(["songname","insong","songinfo","whichsong","rsong","reversesong"]))
# • /whichsong (/songname, /songinfo, /insong, /rsong, /reversesong) : Reply to file to get the song playing in it.
# async def get_song_info(c: Titan, m: Message):
#     if not is_audd:
...
@Titan.on_callback_query(filters.regex("^lyrics_"))
async def lyrics_for_song(c: Titan, q: CallbackQuery):
...
@Titan.on_callback_query(filters.regex("^lyrics_next_") | filters.regex("^lyrics_prev_"))
async def lyrics_for_song_next(c: Titan, q: CallbackQuery):
...
@Titan.on_message(command(["removebackground","removebg","rmbg"]))
async def remove_background(c: Titan, m: Message):
    if not is_rmbg:
...
@Titan.on_message(command(["song","yta"]))
async def song_down_up(c: Titan, m: Message):
    try:
...
@Titan.on_message(command(["vsong","ytv"]))
async def video_down_up(c: Titan, m: Message):
    try:
        splited = m.text.split(None,1)[1].strip()
    except IndexError:
...
@Titan.on_message(command(["ig","instagram","insta"]))
async def download_instareels(c: Titan, m: Message):
    try:
        reel_ = m.command[1]
    except IndexError:
