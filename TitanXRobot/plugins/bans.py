...
from TitanXRobot import LOGGER, MESSAGE_DUMP, OWNER_ID
from TitanXRobot.bot_class import Titan
from TitanXRobot.supports import get_support_staff
from TitanXRobot.utils.caching import ADMIN_CACHE, admin_cache_reload
from TitanXRobot.utils.custom_filters import command, restrict_filter
from TitanXRobot.utils.extract_user import extract_user
from TitanXRobot.utils.extras import BAN_GIFS, KICK_GIFS
from TitanXRobot.utils.parser import mention_html
from TitanXRobot.utils.string import extract_time
from TitanXRobot.vars import Config

SUPPORT_STAFF = get_support_staff()

@Titan.on_message(command("tban") & restrict_filter)
async def tban_usr(c: Titan, m: Message):
...
@Titan.on_message(command("stban") & restrict_filter)
async def stban_usr(c: Titan, m: Message):
    if len(m.text.split()) == 1 and not m.reply_to_message:
        await m.reply_text(text="I can't ban nothing!")
        await m.stop_propagation()
...
@Titan.on_message(command("dtban") & restrict_filter)
async def dtban_usr(c: Titan, m: Message):
    if len(m.text.split()) == 1 and not m.reply_to_message:
        await m.reply_text(text="I can't ban nothing!")
        await m.stop_propagation()
...
@Titan.on_message(command("kick") & restrict_filter)
async def kick_usr(c: Titan, m: Message):
    if len(m.text.split()) == 1 and not m.reply_to_message:
...
@Titan.on_message(command("skick") & restrict_filter)
async def skick_usr(c: Titan, m: Message):
    if len(m.text.split()) == 1 and not m.reply_to_message:
        await m.reply_text(text="I can't kick nothing!")
        return
...
@Titan.on_message(command("dkick") & restrict_filter)
async def dkick_usr(c: Titan, m: Message):
    if len(m.text.split()) == 1 and not m.reply_to_message:
        await m.reply_text(text="I can't ban nothing!")
        return
    if not m.reply_to_message:
...
@Titan.on_message(command("unban") & restrict_filter)
async def unban_usr(c: Titan, m: Message):
    if len(m.text.split()) == 1 and not m.reply_to_message:
...
@Titan.on_message(command("sban") & restrict_filter)
async def sban_usr(c: Titan, m: Message):
    if len(m.text.split()) == 1 and not m.reply_to_message:
        await m.reply_text(text="I can't ban nothing!")
        await m.stop_propagation()
...
@Titan.on_message(command("dban") & restrict_filter)
async def dban_usr(c: Titan, m: Message):
    if len(m.text.split()) == 1 and not m.reply_to_message:
        await m.reply_text(text="I can't ban nothing!")
        await m.stop_propagation()
...
@Titan.on_message(command("ban") & restrict_filter)
async def ban_usr(c: Titan, m: Message):
    if len(m.text.split()) == 1 and not m.reply_to_message:
        await m.reply_text(text="I can't ban nothing!")
        await m.stop_propagation()
...
@Titan.on_callback_query(regex("^unban_"))
async def unbanbutton(c: Titan, q: CallbackQuery):
...
@Titan.on_message(command("kickme"))
async def kickme(c: Titan, m: Message):
    reason = None
    if len(m.text.split()) >= 2:
        reason = m.text.split(None, 1)[1]
    try:
