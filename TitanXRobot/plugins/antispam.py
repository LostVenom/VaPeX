...
from TitanXRobot import LOGGER, MESSAGE_DUMP, SUPPORT_GROUP, TIME_ZONE
...
@Titan.on_message(command(["gban", "globalban"], sudo_cmd=True))
async def gban(c: Titan, m: Message):
...
async def ungban(c: Titan, m: Message):
    if len(m.text.split()) == 1:
