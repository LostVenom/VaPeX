...
from TitanXRobot import LOGGER, SUPPORT_GROUP
...
from TitanXRobot.utils.custom_filters import admin_filter, command
from TitanXRobot.utils.extras import BAN_GIFS, KICK_GIFS, MUTE_GIFS
...
@Titan.on_message(command(['floodaction','actionflood']) & admin_filter)
async def flood_action(c: Titan, m: Message):
    Flood = Floods()
...
@Titan.on_message(command(['isflood', 'flood']) & ~filters.bot)
async def flood_on_off(c: Titan, m: Message):
    if m.chat.type == CT.PRIVATE:   
...
@Titan.on_message(command(['setflood']) & ~filters.bot & admin_filter)
async def flood_set(c: Titan, m: Message):
    bot = await c.get_chat_member(m.chat.id, Config.BOT_ID)
    Flood = Floods()
    status = bot.status
    if not status in [CMS.OWNER, CMS.ADMINISTRATOR]:
...
@Titan.on_callback_query(filters.regex("^f_"))
async def callbacks(c: Titan, q: CallbackQuery):
    data = q.data
    if data == "f_close":
...
@Titan.on_callback_query(filters.regex("^un_"))
async def reverse_callbacks(c: Titan, q: CallbackQuery):
    data = q.data.split("_")
    action = data[1]
...
dic = {}
@Titan.on_message(filters.all & ~filters.bot | ~filters.private, 10)
async def flood_watcher(c: Titan, m: Message):
    c_id = m.chat.id
