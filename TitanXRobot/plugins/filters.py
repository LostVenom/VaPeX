...
from TitanXRobot.bot_class import LOGGER, Titan
...
from TitanXRobot.utils.msg_types import Types, get_filter_type
from TitanXRobot.utils.regex_utils import regex_searcher
...
@Titan.on_message(command("filters") & filters.group & ~filters.bot)
...
@Titan.on_message(command(["filter", "addfilter"]) & admin_filter & ~filters.bot)
...
@Titan.on_message(command(["stop", "unfilter"]) & admin_filter & ~filters.bot)
...
@Titan.on_callback_query(filters.regex("^rm_allfilters$"))
...
async def send_filter_reply(c: Titan, m: Message, trigger: str):
...
@Titan.on_message(filters.text & filters.group & ~filters.bot, group=69)
async def filters_watcher(c: Titan, m: Message):
