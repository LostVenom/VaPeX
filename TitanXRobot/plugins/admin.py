...
from TitanXRobot import LOGGER, OWNER_ID
...
from TitanXRobot.utils.caching import (ADMIN_CACHE, TEMP_ADMIN_CACHE_BLOCK,
                                  admin_cache_reload)
from TitanXRobot.utils.custom_filters import (admin_filter, command, owner_filter,
                                         promote_filter)
...
@Titan.on_message(command("adminlist"))
...
@Titan.on_message(command("zombies") & admin_filter)
async def zombie_clean(c: Titan, m: Message):
    zombie = 0
...
@Titan.on_message(command("admincache"))
...
@Titan.on_message(filters.regex(r"^(?i)@admin(s)?") & filters.group)
...
@Titan.on_message(command("fullpromote") & promote_filter)
async def fullpromote_usr(c: Titan, m: Message):
    global ADMIN_CACHE
    ...
    try:
        ...
        if m.chat.type in [ChatType.SUPERGROUP, ChatType.GROUP]:
            title = "Titan"  # Default fullpromote title
...
@Titan.on_message(command("promote") & promote_filter)
async def promote_usr(c: Titan, m: Message):
    global ADMIN_CACHE
    if len(m.text.split()) == 1 and not m.reply_to_message:
        await m.reply_text(
...
@Titan.on_message(command("demote") & promote_filter)
async def demote_usr(c: Titan, m: Message):
    global ADMIN_CACHE
    if len(m.text.split()) == 1 and not m.reply_to_message:
...
@Titan.on_message(command("invitelink"))
async def get_invitelink(c: Titan, m: Message):
...
@Titan.on_message(command("setgtitle") & admin_filter)
...
@Titan.on_message(command("setgdes") & admin_filter)
...
@Titan.on_message(command("title") & admin_filter)
async def set_user_title(c: Titan, m: Message):
    user = await m.chat.get_member(m.from_user.id)
...
@Titan.on_message(command("setgpic") & admin_filter)
async def setgpic(c: Titan, m: Message):
    user = await m.chat.get_member(m.from_user.id)
    if not user.privileges.can_change_info and user.status != CMS.OWNER:
        await m.reply_text(
            "You don't have enough permission to use this command!",
