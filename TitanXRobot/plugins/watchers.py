...
from TitanXRobot import LOGGER, MESSAGE_DUMP
from TitanXRobot.bot_class import Titan
from TitanXRobot.database.antispam_db import ANTISPAM_BANNED, GBan
from TitanXRobot.database.approve_db import Approve
from TitanXRobot.database.blacklist_db import Blacklist
from TitanXRobot.database.group_blacklist import BLACKLIST_CHATS
...
@Titan.on_message(filters.linked_channel)
async def antichanpin_cleanlinked(c: Titan, m: Message):
    try:
        msg_id = m.id
...
@Titan.on_message(filters.text & filters.group, group=5)
...
@Titan.on_message(filters.user(list(ANTISPAM_BANNED)) & filters.group)
async def gban_watcher(c: Titan, m: Message):
    from TitanXRobot import SUPPORT_GROUP
...
@Titan.on_message(filters.chat(BLACKLIST_CHATS))
async def bl_chats_watcher(c: Titan, m: Message):
    from TitanXRobot import SUPPORT_GROUP
