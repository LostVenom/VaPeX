...
from TitanXRobot.database.locks_db import LOCKS
...
@Titan.on_message(command("locktypes"))
...
@Titan.on_message(command("lock") & restrict_filter)
async def lock_perm(c: Titan, m: Message):
...
@Titan.on_message(command("locks") & restrict_filter)
...
@Titan.on_message(command("unlock") & restrict_filter)
async def unlock_perm(c: Titan, m: Message):
    if len(m.text.split()) < 2:
...
async def delete_messages(c:Titan, m: Message):
    try:
        await m.delete()
        return
...
async def is_approved_user(c:Titan, m: Message):
...
@Titan.on_message(filters.group & ~filters.me,18)
async def lock_del_mess(c:Titan, m: Message):
    lock = LOCKS()
