...
from TitanXRobot.utils.sticker_help import *
from TitanXRobot.utils.web_helpers import get_file_size
from TitanXRobot.vars import Config


@Titan.on_message(command(["stickerinfo","stinfo"]))
async def give_st_info(c: Titan , m: Message):
...
@Titan.on_message(command(["stickerid","stid"]))
async def sticker_id_gib(c: Titan, m: Message):
    if not m.reply_to_message:
        await m.reply_text("Reply to a sticker")
        return
    elif not m.reply_to_message.sticker:
...
@Titan.on_message(command(["kang", "steal"]))
async def kang(c:Titan, m: Message):
    if not m.reply_to_message:
...
@Titan.on_message(command(["mmfb","mmfw","mmf"]))
async def memify_it(c: Titan, m: Message):
    if not m.reply_to_message:
...
@Titan.on_message(command(["getsticker","getst"]))
async def get_sticker_from_file(c: Titan, m: Message):
