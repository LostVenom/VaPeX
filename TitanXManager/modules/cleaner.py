...
from TitanXManager import ALLOW_EXCL, CustomCommandHandler, dispatcher
...
from TitanXManager.modules.sql import cleaner_sql as sql
...
def set_blue_text_must_click(update: Update, context: CallbackContext):
    ...
    if len(args) >= 1:
        ...
        if val in ("off", "no"):
            ...
            reply = "Commands cleaning has been disabled for <b>{}</b>".format(
            ...
            reply = "Commands cleaning has been enabled for <b>{}</b>".format(
                html.escape(chat.title)
            )
            message.reply_text(reply, parse_mode=ParseMode.HTML)
        ...
        reply = "Commands cleaning for <b>{}</b> : <b>{}</b>".format(
...
def add_bluetext_ignore(update: Update, context: CallbackContext):
    ...
    if len(args) >= 1:
        ...
        if added:
            reply = "<b>{}</b> has been added to CL Clean ignore list.".format(
                args[0]
            )
        else:
...
def remove_bluetext_ignore(update: Update, context: CallbackContext):
    ...
    if len(args) >= 1:
        ...
        if removed:
            reply = (
                "<b>{}</b> has been removed from CL Clean ignore list.".format(
                    args[0]
                )
            )
        else:
...
def add_bluetext_ignore_global(update: Update, context: CallbackContext):
    ...
    if len(args) >= 1:
        ...
        if added:
            reply = "<b>{}</b> has been added to global CL Clean ignore list.".format(
                args[0]
            )
        else:
            reply = "Command is already ignored."
...
def remove_bluetext_ignore_global(update: Update, context: CallbackContext):
    ...
    if len(args) >= 1:
        ...
        if removed:
            reply = "<b>{}</b> has been removed from global CL Clean ignore list.".format(
                args[0]
            )
        else:
            reply = "Command isn't ignored currently."
...
def bluetext_ignore_list(update: Update, context: CallbackContext):
    ...
    if global_ignored_list:
        text = "The following commands are currently ignored globally from CL Clean :\n"

        for x in global_ignored_list:
            text += f" - <code>{x}</code>\n"

    if local_ignore_list:
        text += "\nThe following commands are currently ignored locally from CL Clean :\n"

        for x in local_ignore_list:
            text += f" - <code>{x}</code>\n"

    if text == "":
        text = "No commands are currently ignored from CL Clean."
...
__help__ = """
Removes any Commands that peoples send in chat.

 ✧ /cleancommand <on/off/yes/no>*:* Clean Commands after sent in Group.
 ✧ /ignorecommand <word>*:* Prevents given commamd from auto cleaning.
 ✧ /unignorecommand <word>*:* Removes prevented commamds from ignore list.
 ✧ /listcommamds*:* List all currently whitelisted commands.
"""

SET_CLEAN_BLUE_TEXT_HANDLER = CommandHandler(
    "cleancommand", set_blue_text_must_click, run_async=True
)
ADD_CLEAN_BLUE_TEXT_HANDLER = CommandHandler(
    "ignorecommand", add_bluetext_ignore, run_async=True
)
REMOVE_CLEAN_BLUE_TEXT_HANDLER = CommandHandler(
    "unignorecommand", remove_bluetext_ignore, run_async=True
)
ADD_CLEAN_BLUE_TEXT_GLOBAL_HANDLER = CommandHandler(
    "gignorecommand", add_bluetext_ignore_global, run_async=True
)
REMOVE_CLEAN_BLUE_TEXT_GLOBAL_HANDLER = CommandHandler(
    "ungignorecommand", remove_bluetext_ignore_global, run_async=True
)
LIST_CLEAN_BLUE_TEXT_HANDLER = CommandHandler(
    "listcommands", bluetext_ignore_list, run_async=True
)
