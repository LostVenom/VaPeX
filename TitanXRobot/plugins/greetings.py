...
from TitanXRobot import LOGGER
from TitanXRobot.bot_class import Titan
from TitanXRobot.database.antispam_db import GBan
from TitanXRobot.database.greetings_db import Greetings
from TitanXRobot.supports import get_support_staff
from TitanXRobot.utils.cmd_senders import send_cmd
from TitanXRobot.utils.custom_filters import admin_filter, bot_admin_filter, command
from TitanXRobot.utils.kbhelpers import ikb
from TitanXRobot.utils.msg_types import Types, get_wlcm_type
from TitanXRobot.utils.parser import escape_markdown, mention_html
from TitanXRobot.utils.string import (build_keyboard, escape_invalid_curly_brackets,
                                 parse_button)
...
@Titan.on_message(command("cleanwelcome") & admin_filter)
...
@Titan.on_message(command("cleangoodbye") & admin_filter)
...
@Titan.on_message(command("cleanservice") & admin_filter)
...
@Titan.on_message(command("setwelcome") & admin_filter & bot_admin_filter)
...
@Titan.on_message(command("setgoodbye") & admin_filter & bot_admin_filter)
...
@Titan.on_message(command("resetgoodbye") & admin_filter & bot_admin_filter)
...
@Titan.on_message(command("resetwelcome") & admin_filter & bot_admin_filter)
...
@Titan.on_message(filters.service & filters.group, group=59)
...
@Titan.on_chat_member_updated(filters.group, group=69)
async def member_has_joined(c: Titan, member: ChatMemberUpdated):
...
@Titan.on_chat_member_updated(filters.group, group=99)
async def member_has_left(c: Titan, member: ChatMemberUpdated):
...
@Titan.on_message(command("welcome") & admin_filter)
async def welcome(c: Titan, m: Message):
    db = Greetings(m.chat.id)
    status = db.get_welcome_status()
    oo = db.get_welcome_text()
    args = m.text.split(" ", 1)
...
@Titan.on_message(command("goodbye") & admin_filter)
async def goodbye(c: Titan, m: Message):
    db = Greetings(m.chat.id)
    status = db.get_goodbye_status()
    oo = db.get_goodbye_text()
    args = m.text.split(" ", 1)
...
× Titan must be an admin to greet and goodbye users.
