...
import TitanXManager.modules.sql.blacklist_sql as sql
from TitanXManager import LOGGER, dispatcher
from TitanXManager.modules.connection import connected
from TitanXManager.modules.disable import DisableAbleCommandHandler
from TitanXManager.modules.helper_funcs.alternate import send_message, typing_action
from TitanXManager.modules.helper_funcs.chat_status import user_admin, user_not_admin
from TitanXManager.modules.helper_funcs.extraction import extract_text
from TitanXManager.modules.helper_funcs.misc import split_message
from TitanXManager.modules.helper_funcs.string_handling import extract_time
from TitanXManager.modules.log_channel import loggable
from TitanXManager.modules.sql.approve_sql import is_approved
from TitanXManager.modules.warns import warn
...
def blacklist(update, context):
    ...
    filter_list = "Currently blacklisted words in <b>{}</b>:\n".format(chat_name)
    ...
    for text in split_text:
        if filter_list == "Currently blacklisted words in <b>{}</b>:\n".format(
            html.escape(chat_name)
        ):
            send_message(
                update.effective_message,
                "No any blacklisted word in <b>{}</b>!".format(html.escape(chat_name)),
...
def add_blacklist(update, context):
    ...
    if len(words) > 1:
        ...
        if len(to_blacklist) == 1:
            ...
            send_message(
                update.effective_message,
                "Added to blacklist : <code>{}</code> in <b>{}</b>!".format(
        ...
        send_message(
            update.effective_message,
            "what do you want to add in blacklist.",
        )
    ...
    if len(words) > 1:
        ...
        if len(to_unblacklist) == 1:
            if successful:
                ...
                send_message(
                    update.effective_message, "This is not in blacklist!"
                )
            ...
            send_message(
                update.effective_message,
                "None of these exist in blaclist so it can't be removed.",
                parse_mode=ParseMode.HTML,
            )
        ...
        send_message(
            update.effective_message,
            "what do you want to remove from blacklist!",
        )
...
__help__ = """
Blacklists are basically used for stopping certain words from being told in groups.If anyone tries to send any blacklisted thing in group it will be deleted immediately.
...
Admin only:
 ✧ /addblacklist <blacklist word><reason>*:* Add to the blacklist.An entire sentence can be blocklisted by putting it in " " .
 ✧ /unblacklist <blacklist word>*:* Remove from the blacklist.
 ✧ /blacklistmode <off/del/warn/ban/kick/mute/tban/tmute>*:* Action to perform when someone sends blacklisted words.
 ✧ /blacklistdel <on/off/yes/no>*:* Set whether blaclisted message should be deleted or not.
"""
