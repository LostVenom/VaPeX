...
@Titan.on_callback_query(filters.regex("^formatting."))
async def get_formatting_info(c: Titan, q: CallbackQuery):
    cmd = q.data.split(".")[1]
...
Use this to make Titan feel a bit more customised! (only works in filters/notes)
...
@Titan.on_callback_query(filters.regex("^back."))
async def send_mod_help(_, q: CallbackQuery):
    await q.edit_message_caption(
        caption="""Formatting

Titan supports a large number of formatting options to make your messages more expressive. Take a look by clicking the buttons below!""",
