...
async def gdpr_remove(_, m: Message):
    ...
    await m.reply_text(
        ...
        "Note that this will not unban you from any chats, as that is telegram data, not Titan data."
...
async def id_info(c: Titan, m: Message):
...
@Titan.on_message(command("paste"))
...
@Titan.on_message(command("bug"))
async def reporting_query(c: Titan, m: Message):
