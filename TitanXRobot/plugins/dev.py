...
from TitanXRobot import (BOT_TOKEN, LOG_DATETIME, LOGFILE, LOGGER, MESSAGE_DUMP,
                    OWNER_ID, UPTIME)
from TitanXRobot.bot_class import Titan
from TitanXRobot.database import MongoDB
from TitanXRobot.database.chats_db import Chats
from TitanXRobot.database.support_db import SUPPORTS
from TitanXRobot.database.users_db import Users
from TitanXRobot.plugins.scheduled_jobs import clean_my_db
from TitanXRobot.supports import get_support_staff
from TitanXRobot.utils.clean_file import remove_markdown_and_html
from TitanXRobot.utils.custom_filters import command
from TitanXRobot.utils.extract_user import extract_user
from TitanXRobot.utils.parser import mention_markdown
...
@Titan.on_message(command(["addsupport"]))
async def add_support(c: Titan, m:Message):
    support = SUPPORTS()
...
@Titan.on_message(command("rmsupport"))
async def rm_support(c: Titan, m: Message):
    support = SUPPORTS()
    curr_user = support.get_support_type(m.from_user.id)
    if not curr_user:
        await m.reply_text("Stay in you limit")
...
@Titan.on_message(command("ping", sudo_cmd=True))
...
@Titan.on_message(command("logs", dev_cmd=True))
async def send_log(c: Titan, m: Message):
...
@Titan.on_message(command("neofetch", dev_cmd=True))
...
@Titan.on_message(command(["eval", "py"], dev_cmd=True))
async def evaluate_code(c: Titan, m: Message):
...
@Titan.on_message(command(["exec", "sh"], dev_cmd=True))
async def execution(c: Titan, m: Message):
    protect = BOT_TOKEN.split(":")
    initial = protect[0]
    end = protect[1][-5:]
    if len(m.text.split()) == 1:
...
async def stop_and_send_logger(c:Titan,is_update=False):
...
@Titan.on_message(command(["restart", "update"], owner_cmd=True),group=-100)
async def restart_the_bot(c:Titan,m:Message):
    try:
        cmds = m.command
        ...
        execvp(executable, [executable, "-m", "TitanXRobot"])
    except Exception as e:
...
@Titan.on_message(command("chatlist", dev_cmd=True))
async def chats(c: Titan, m: Message):
...
@Titan.on_message(command("uptime", dev_cmd=True))
...
@Titan.on_message(command("leavechat", dev_cmd=True))
async def leave_chat(c: Titan, m: Message):
...
@Titan.on_message(command("chatbroadcast", dev_cmd=True))
async def chat_broadcast(c: Titan, m: Message):
    if m.reply_to_message:
...
@Titan.on_message(command(["forward","fwd"],dev_cmd=True))
async def forward_type_broadcast(c: Titan, m: Message):
...
@Titan.on_message(command(["cleandb","cleandatabase"],sudo_cmd=True))
async def cleeeen(c:Titan,m:Message):
