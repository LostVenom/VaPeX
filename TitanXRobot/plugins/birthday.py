...
from TitanXRobot import BDB_URI, LOGGER, TIME_ZONE
from TitanXRobot.bot_class import Titan
from TitanXRobot.database.chats_db import Chats

if BDB_URI:
    from TitanXRobot.plugins import bday_cinfo, bday_info
...
@Titan.on_message(command("remember"))
async def remember_me(c: Titan, m: Message):
    if not BDB_URI:
...
@Titan.on_message(command(["removebday","rmbday"]))
async def who_are_you_again(c: Titan, m: Message):
    if not BDB_URI:
        await m.reply_text("BDB_URI is not configured")
        return
    user = m.from_user.id
...
@Titan.on_message(command(["nextbdays","nbdays","birthdays","bdays"]))
async def who_is_next(c: Titan, m: Message):
    if not BDB_URI:
        await m.reply_text("BDB_URI is not configured")
        return
...
@Titan.on_message(command(["getbday","gbday","mybirthday","mybday"]))
async def cant_recall_it(c: Titan, m: Message):
    if not BDB_URI:
        await m.reply_text("BDB_URI is not configured")
        return
    user = m.from_user.id
...
@Titan.on_message(command(["settingbday","sbday"]))
async def chat_birthday_settings(c: Titan, m: Message):
    if not BDB_URI:
        await m.reply_text("BDB_URI is not configured")
        return
    if m.chat.type == ChatType.PRIVATE:
...
@Titan.on_callback_query(filters.regex("^switchh_"))
async def switch_on_off(c:Titan, q: CallbackQuery):
