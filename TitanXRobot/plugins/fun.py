...
from TitanXRobot.utils import extras
from TitanXRobot.utils.custom_filters import command
from TitanXRobot.utils.extras import NOWYES as NO
from TitanXRobot.utils.extras import YESWNO as YES

DEV_USERS = get_support_staff("dev")

@Titan.on_message(command("shout"))
...
@Titan.on_message(command("slap"))
async def fun_slap(c: Titan, m: Message):
    me = await c.get_me()
    ...
    if user_id == me.id:
        temp = choice(extras.SLAP_TITAN_TEMPLATES)
    else:
...
@Titan.on_message(command("insult"))
async def insult(c: Titan, m: Message):
...
@Titan.on_message(command("yes"))
async def yesw(c: Titan, m: Message):
    reply_text = m.reply_to_message.reply_text if m.reply_to_message else m.reply_text
    rtext = YES[0]
...
@Titan.on_message(command("no"))
async def now(c: Titan, m: Message):
    reply_text = m.reply_to_message.reply_text if m.reply_to_message else m.reply_text
    rtext = NO[0]
    await reply_text(rtext)
...
@Titan.on_message(command("shrug"))
...
@Titan.on_message(command("bluetext"))
...
@Titan.on_message(command("decide"))
...
@Titan.on_message(command("react"))
...
@Titan.on_message(command("weebify"))
